package mihon.feature.airingschedule.notification
    ...
    *
    * Notification style (compact card — consistent across all Android versions):
    *  - Cover art is set as the large icon, which Android renders as a right-side thumbnail
    *    in the compact collapsed card. This is intentional: BigPictureStyle is NOT used
    *    because it always expands the notification into a wide banner image, which looks
    *    different from the desired compact thumbnail layout.
    *  - Text uses BigTextStyle so long titles can still expand without triggering a banner.
    *  - An "Open" action button is added at the bottom of the notification.
    */
      ...
      private fun postNotification(
          ...
          runCatching {
              context.notify(notificationId, Notifications.CHANNEL_AIRING_SCHEDULE) {
                  ...
                  // Cover art as a right-side thumbnail in the compact card.
                  // Using setLargeIcon (not BigPictureStyle) keeps the notification compact
                  // on every Android version — no banner image, just the thumbnail.
                  if (coverBitmap != null) {
                      setLargeIcon(coverBitmap)
                  }

                  // BigTextStyle only for text expansion — never shows a banner image.
                  setStyle(NotificationCompat.BigTextStyle().bigText(contentText))

                  // "Open" action button — tapping it launches the anime detail screen.
                  addAction(
